...
setwd("/Users/annabelhughes/Documents/Davies\ Lab/Git/Reciprocal_Transplants")
...
df <- read.table("219_20220803T065042_DBV_20220803T143825.profiles.absolute.abund_and_meta.txt", sep = "\t")[c(-1:-6,-37, -38),-1] %>%
  row_to_names(row_number = 1)
names(df)[1] <- "sampleID"
write.csv(df, "ITS2_symportal_RT.csv", row.names = FALSE)
```

```{r read in ITS2 data}
## read in ITS2 data
its_df <- read.csv("ITS2_symportal_RT.csv") %>% 
...
ggsave("FigSXX_ITS2_RT.png", height = 4, width = 6, dpi = 600)
ggsave("FigSXX_ITS2_RT.pdf", height = 4, width = 6, useDingbats = FALSE)
...
**Figure SXX.** Relative abundance of major ITS2 types by coral fragment (x axis).
