...
temp_long <- read.csv("RT_data/RTE_Temperature_Nov2014_Oct2015.csv") %>% 
...
temp_wide <- read.csv("RT_data/RTE_Temperature_Nov2014_Oct2015.csv") %>% 
  mutate(date_time = as.POSIXct(Date.Time, format = "%m/%d/%y %H:%M")) %>% # convert date/time to POSIXct class 
  select(-Date.Time) %>% 
  drop_na()
...
ggplot(data = temp_long, aes(x = date_time, y = temp, colour = reef, group = reef)) +
  geom_line() +
...
ggsave("RT_figures/Overall_temp.pdf", height = 4, width = 4, useDingbats = FALSE) # save plot
...
ggsave("RT_figures/Mean_temp.pdf", height = 4, width = 12, useDingbats = FALSE) # save plot
...
ggsave("RT_figures/Range_temp.pdf", height = 4, width = 12, useDingbats = FALSE) # save plot
...
sur.all <- survfit(sur.r ~ Transplant + Source, data = sur) # Computes an estimate of a survival curve based on transplantation using Kaplan-Meier; this will also be used for plotting
...
surv_plot <- ggsurvplot(sur.all, data = sur,
           palette = c("#0072B2", "#0072B2","#0072B2","#009E73","#009E73","#009E73","#D55E00","#D55E00","#D55E00"),
           linetype = c(1, 2, 3, 1, 2, 3, 1, 2, 3),
           censor = FALSE,
           legend.title = "",
           legend.labs = c("BR to BR", "FR to BR", "NS to BR", "BR to FR", "FR to FR", "NS to FR", "BR to NS", "FR to NS", "NS to NS")) 
...
## Save survival plots
# ggsave("RT_figures/Figure2A_Survival.pdf", width = 5.7, height = 5.5, useDingbats = FALSE) # save as PDF
# ggsave("RT_figures/Figure2A_Survival.png", width = 5.7, height = 5.5) # save as PNG
...
tableS4 <- read.csv("RT_data/RT_survival_table.csv")[1:7] # Read in num corals surviving at each timepoint
...
RT.calc <- read.csv("RT_output/RT_calc_boot.csv", header = TRUE) # new dataframe with modeled 95% CI and mean calcification rates
...
RT.calc.tp <- read.csv("RT_output/RT_TP_calc_boot.csv", header = TRUE) # new dataframe with modeled 95% CI and mean calcification rates
...
ggarrange(fig_s2a, fig_s2b, fig_s2c, fig_s2d, fig_s2e, ncol = 3, nrow = 2, labels = "AUTO")
ggsave("RT_figures/FigureS2_temps.pdf", height = 8, width = 14, useDingbats = FALSE) # save plot
ggsave("RT_figures/FigureS2_temps.png", height = 8, width = 14) # save plot
...
plot_grid(fig2b, calc.tp2.plot, rel_widths = c(0.7,1.3), labels = c("A", "B"), scale = 0.9) +
...
map_png <- readPNG("RT_figures/Figure1A_Map.png")
...
ggsave("RT_figures/Figure1_map_temp.pdf", height = 4, width = 13, useDingbats = FALSE) # save plot as PDF
ggsave("RT_figures/Figure1_map_temp.png", height = 4, width = 13) # save plot as PNG
...
## Save survival plots
ggsave("RT_figures/Figure2_survival_calc.pdf", width = 11, height = 5, useDingbats = FALSE) # save as PDF
ggsave("RT_figures/Figure2_survival_calc.png", width = 11, height = 5) # save as PNG
