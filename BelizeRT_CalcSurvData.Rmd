...
```{r calc df setup, echo=FALSE, message=FALSE, warning=FALSE}

# read in dataframe
RT.calc <- read.csv('/Users/colleen/BelizeRT_CalcSurvData/RT_data/RT_calcification.csv') # read in data
...
**Table 1 | Overall calcification Akaike information criterion (AIC) model selection**
...
```{r calcification AIC model selection, echo=FALSE, message=FALSE, warning=FALSE}
...
Model:
```{r calcification model, message=FALSE, warning=FALSE}
...
colnames(RT.calc.fin2)[6:8] <- c("mean", "lowerci", "upperci")

write.csv(RT.calc.fin2, "/Users/colleen/BelizeRT_CalcSurvData/RT_output/RT_calc_boot.csv")

```

<br/>

**Table 2 | Modelled Mean Calcification Rate and 95% Confidence Interval**
...
```{r CI table, echo=FALSE, message=FALSE, warning=FALSE}

RT.calc <- read.csv("/Users/colleen/BelizeRT_CalcSurvData/RT_output/RT_calc_boot.csv", header = TRUE) # new dataframe with modelled 95% CI and mean calcification rates
...
RT.calc$Treatment <- paste(RT.calc$source, RT.calc$trans, sep = "_to_") # make a 'treatment' column

# condensed dataframe of modelled mean and 95% CI per treatment
RT.calc.tab <- RT.calc %>% 
  group_by(Treatment) %>% 
  summarize(`Modelled Mean` = mean(mean, na.rm=TRUE), `Lower 95% CI` = mean(lowerci, na.rm=TRUE), `Upper 95% CI` = mean(upperci, na.rm=TRUE))
...
```{r calcification plot, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=5}
...
**Table 3 | Time point calcification Akaike information criterion (AIC) model selection**
...
```{r TP calcification AIC model selection, echo=FALSE, message=FALSE, warning=FALSE}
...
Model:
```{r TP calcification model, message=FALSE, warning=FALSE}
...
colnames(RT.calc.tp2)[6:8] <- c("mean", "lowerci", "upperci")

write.csv(RT.calc.tp2, "/Users/colleen/BelizeRT_CalcSurvData/RT_output/RT_TP_calc_boot.csv")
...
**Table 4 | Modelled Mean Calcification Rate and 95% Confidence Interval by Time Point**
...
```{r TP CI table, echo=FALSE, message=FALSE, warning=FALSE}

RT.calc.tp <- read.csv("/Users/colleen/BelizeRT_CalcSurvData/RT_output/RT_TP_calc_boot.csv", header = TRUE) # new dataframe with modelled 95% CI and mean calcification rates
...
RT.calc.tp$Treatment <- paste(RT.calc.tp$source, RT.calc.tp$trans, sep = "_to_") # make a 'treatment' column
...
```{r TP calcification plot, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=4, fig.width=9.5}
...
calc.tp.plot <- ggplot(data = RT.calc.tp) +
  ...
  facet_wrap(~ source, labeller = labeller(source = c("NS" = "Nearshore", "BR" = "Backreef", "FR" ="Forereef"))) +
...
**Figure 2B |** Calcification rate (mg cm^-2^ day^-1^) at yearly intervals over the entire transplant experimental period (2012 – 2015) by source location. Transplant location is represented by color: <span style="color: #D55E00;">red = NS (nearshore)</span>, <span style="color: #009E73;">green = BR (backreef)</span>, and <span style="color: #0072B2;">blue = FR (forereef)</span>. Colored bars represent modelled 95% confidence intervals with corresponding raw calcification rates per colony denoted by circles of the same color. 
...
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.height=3.3, fig.width=12}

plot_grid(calc.avg.plot, calc.tp.plot, rel_widths = c(0.7,1.3), labels = c("A", "B"), scale = 0.9) +
  ggsave("/Users/colleen/BelizeRT_CalcSurvData/RT_figures/Figure2_Calcification_modified.pdf", height = 3.3, width = 12)
...
```{r survival dataframe setup, echo=FALSE, message=FALSE, warning=FALSE}

sur <- read.csv("/Users/colleen/BelizeRT_CalcSurvData/RT_data/RT_survival.csv") # Read in survival data (0 = living, 1 = mortality event under 'Status')
...
sur.all <- survfit(sur.r ~ Tranplant + Source, data = sur) 
...
```{r survival model table, echo=FALSE, message=FALSE, warning=FALSE}
surv.tab <- anova(surv.mod)  

kable(surv.tab, digits = 3) %>% 
  kable_styling(position = "center", font_size = 14, full_width = FALSE) %>% 
  row_spec(0, background = "lightgrey")
...
```{r survival figure, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', fig.width= 5.5, fig.height=6}

plot(sur.all, col = c("#0072B2", "#0072B2","#0072B2","#009E73","#009E73","#009E73","#D55E00","#D55E00","#D55E00"), lwd = c(5, 5, 5, 4, 4, 4, 3, 3, 3), lty = c(1, 2, 3, 1, 2, 3, 1, 2, 3), xaxt = "n", xlab = "Days since transplant", ylab = "Fraction surviving")
axis(side = 1, at = c(0, 365, 711, 1088, 1445)) # Creates the axis label for x axis
...
pdf("/Users/colleen/BelizeRT_CalcSurvData/RT_figures/Figure3_Survival_modification.pdf", width = 5.5, height = 5.7) # writes figure as a PDF to file
plot(sur.all, col = c("#0072B2", "#0072B2","#0072B2","#009E73","#009E73","#009E73","#D55E00","#D55E00","#D55E00"), lwd = c(5, 5, 5, 4, 4, 4, 3, 3, 3), lty = c(1, 2, 3, 1, 2, 3, 1, 2, 3), xaxt = "n", xlab = "Days since transplant", ylab = "Fraction surviving")
axis(side = 1, at = c(0, 365, 711, 1088, 1445))
legend("bottomleft", col = c("#0072B2", "#009E73","#D55E00"), legend = c("FR","BR","NS"), title = "Transplant", lwd = 3, bty = "n")
...
**Figure 3.** Fraction surviving of *Siderastrea siderea* colonies across time (2011-2015) with respect to source location and transplanted location. Source location is represented by line type (FR = dashed; BF = solid; NS = dotted) and transplant location is denoted by colour (<span style="color: #D55E00;">red = NS (nearshore)</span>, <span style="color: #009E73;">green = BR (backreef)</span>, and <span style="color: #0072B2;">blue = FR (forereef)</span>). 
